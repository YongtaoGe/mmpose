...
model = dict(
    ...
    neck=dict(type='OneQueryInputProj', in_channels=(256, 256, 256, 256), out_channel=256, num_joints=17),
    keypoint_head=dict(
        type='OneQueryTransHead',
        num_joints=channel_cfg['num_output_channels'],
        # loss_keypoint=dict(type='SmoothL1Loss', use_target_weight=True, loss_weight=1000),
        loss_hp_keypoint=[dict(type='JointsMSELoss', use_target_weight=True, loss_weight=0.25)] * 3 +
            [dict(type='JointsOHKMMSELoss', use_target_weight=True, loss_weight=1.)] * 1,
        loss_coord_keypoint=dict(type='L1Loss', use_target_weight=True, loss_weight=40),
        in_channels=2048,
        out_indices=(0, 1, 2, 3),
