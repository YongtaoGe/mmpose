...
class PoseurHead(BaseHead):
    ...
    def __init__(self,
        ...
        self.use_dec_rle_loss = isinstance(self.loss_coord_dec, RLELoss_poseur) or isinstance(self.loss_coord_dec, RLEOHKMLoss)
    ...
    def forward(self, mlvl_feats, coord_target=None, coord_target_weight=None):
        ...
        enc_outputs = self.transformer(
            ...
            coord_target_weight=coord_target_weight
        )
        ...
        if self.use_heatmap_loss:
            ...
            outputs_hp = self.final_layer(x)
            dec_outputs.update(hp=outputs_hp)
            ...
            if self.use_dec_rle_loss:
                for i, (pred_jts, sigma) in enumerate(zip(output.pred_jts, output.sigma)):
                    ...
                    log_phi = self.dec_flow.log_prob(bar_mu.reshape(-1, 2)).reshape(BATCH_SIZE, 
                                        self.num_joints * (self.transformer.num_noise_sample+1), 1)
    ...
    def predict(self, x, flip_pairs=None, coord_target=None, coord_target_weight=None):
        """Inference function.
        ...
        # import pdb
        # pdb.set_trace()
        output_enc, output_dec = self.forward(x, coord_target, coord_target_weight)
        # coord_output = output["coord"]
